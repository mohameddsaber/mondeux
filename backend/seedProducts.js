...
Every product comes in a recyclable pouch. Our pouches are made from 100% cotton with cotton drawstrings and are completely plastic free & recyclable. Silver products (excluding Rhodium plated styles) naturally age over time, so they also come with a cleaning cloth, which can be used to polish your product.`,
    ...
    variants: [
      {
        attribute: { type: 'material', value: 'silver', meta: '925' },
        images: [],
        price: 1800,
        compareAtPrice: 2200,
        costPrice: 1100,
      ...
      {
        attribute: { type: 'material', value: 'stainless steel' },
        images: [],
        price: 450,
        compareAtPrice: 550,
        costPrice: 200,
...
const seedProducts = async () => {
  try {
    ...
    for (const productData of productsData) {
      ...
      const variants = productData.variants.map((mv) => {
        const sizeVariants = mv.sizeVariants.map((sv) => ({
          ...sv,
          sku: generateSKU(slug, mv.attribute.value, sv.label),
      ...
      console.log(`✅ Created: ${productData.name} (${variants.length} materials)`);
    }
